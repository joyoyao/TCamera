...
public class GlTextureView extends TextureView implements TextureView.SurfaceTextureListener{
    ...
    public GlTextureView(@NonNull final Context context) {
        super(context);
        init(context);
    }

    public GlTextureView(@NonNull final Context context, final AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public GlTextureView(@NonNull final Context context, final AttributeSet attrs, final int defStyle) {
    ...
    @Override
    public void setSurfaceTextureListener(final TextureView.SurfaceTextureListener listener) {
                    ...
                    synchronized (sGLThreadManager) {
                        while (true) {
                            ...
                            if (pausing && mHaveEglContext) {
                                final GlTextureView view = mGLSurfaceViewWeakRef.get();
                                final boolean preserveEglContextOnPause = view != null && view.mPreserveEGLContextOnPause;
                            ...
                            if (readyToDraw()) {
                                ...
                                if (! mHaveEglContext) {
                                    if (askedToReleaseEglContext) {
                                        ...
                                        try {
                                            mEglHelper.start();
                                        } catch (@NonNull final RuntimeException t) {
                                            sGLThreadManager.releaseEglContextLocked(this);
                                            throw t;
                                        }
    ...
    private static class GLThreadManager {
        ...
        public synchronized void threadExiting(@NonNull final GLThread thread) {
            if (LOG_THREADS) {
        ...
        public synchronized void checkGLDriver(@NonNull final GL10 gl) {
            if (!mGLESDriverCheckComplete) {
                ...
                mLimitedGLESContexts = !mMultipleGLESContextsAllowed || (Build.VERSION.SDK_INT < JELLY_BEAN && renderer.startsWith(kADRENO));
                if (LOG_SURFACE) {
